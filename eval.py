...
# E-FineR
use_context_for_filtration = False  # use generated incotext sentences for voting
use_context_for_classification = False  # use generated incotext sentences for final classifier

# FiNDR
lmm_discovered_classnames = True  # use LMM discovered class-names
if lmm_discovered_classnames:
    llm_name = "qwen"  # options: ["qwen", "gemini"]
...
def generate_voted_classifier(
    ...
    if lmm_discovered_classnames:
        lmm_class_names_path = f"./data/guessed_classnames/{cfg['dataset_name']}{cfg['num_classes']}/{cfg['dataset_name']}s_{llm_name}.json"
    ...
    if lmm_discovered_classnames:
        # load json with generations:
        with open(lmm_class_names_path, 'r') as f:
            classnames_generated_full = json.load(f)

        generated_classnames = []
        bad_generations = 0
        for image_path, image_prediction in classnames_generated_full.items():
            if top1_only:
                if (image_prediction['model_main_success'] == True) and (image_prediction['model_service_success'] == True):
                    if "0" in image_prediction['guessed_classnames']:
                        starting_idx = "0"
                    elif "1" in image_prediction['guessed_classnames']:
                        starting_idx = "1"
                    elif "a" in image_prediction['guessed_classnames']:
                        starting_idx = "a"
                    else:
                        raise ValueError(f"Incorrect starting index found in guessed_classnames keys: {image_prediction['guessed_classnames'].keys()}, for image {image_path}")
                    
                    classname_generated = image_prediction['guessed_classnames'][starting_idx]
                    classname_generated = classname_generated.replace("*", "")
                    classname_generated = classname_generated.replace("_", "")
                    classname_generated = classname_generated.replace("'", "")
                    classname_generated = classname_generated.replace('"', '')

                    generated_classnames.append(classname_generated) # take only the top-1 prediction
                else:
                    bad_generations += 1
                    continue
            else:
                if (image_prediction['model_main_success'] == "true") and (image_prediction['model_service_success'] == "true"):
                    for idx, name in image_prediction['guessed_classnames'].items():
                        name = name.replace("*", "")
                        name = name.replace("_", "")
                        generated_classnames.append(name)
                else:
                    bad_generations += 1
                    continue
        
        print("Number of bad generations: ", bad_generations)
        print("Number of generated_classnames before: ", len(generated_classnames))
        print(generated_classnames)

        # remove duplicates:
        generated_classnames = list(set(generated_classnames))
        print("Number of generated_classnames after: ", len(generated_classnames))
        print(generated_classnames)

        guessed_cnames = generated_classnames
    ...
    if do_robustness_test:
        ...
        for i in range(num_to_replace):
            index = i * (len(guessed_cnames) // num_to_replace)

            if robustness_test_type == "generic_name":
                guessed_cnames[index] = "dog breed" if (cfg['dataset_name']+"s") == "dogs" \
                                    else "car model" if (cfg['dataset_name']+"s") == "cars" \
                                    else "flower species" if (cfg['dataset_name']+"s") == "flowers" \
                                    else "bird species" if (cfg['dataset_name']+"s") == "birds" \
                                    else "pet type" if (cfg['dataset_name']+"s") == "pets" \
                                    else "object"
            elif robustness_test_type == "misprediction": 
                # swap with random class (except itself)
                import random
                guessed_cnames[index] = random.choice([x for x in guessed_cnames if x != guessed_cnames[index]])
            elif robustness_test_type == "random noise":
                import random
                import string
                random_str = ''.join(random.choices(string.ascii_lowercase + string.digits, k=5))
                guessed_cnames[index] = random_str
            else:
                raise ValueError(f"Unknown robustness_test_type {robustness_test_type}")
...
if __name__ == "__main__":
    ...
    if upper_bound or lmm_discovered_classnames:
        guessed_cnames = None
    else:
